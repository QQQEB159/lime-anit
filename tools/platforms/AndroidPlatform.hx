...
class AndroidPlatform extends PlatformTarget
{
	...
	private function cleanEmptyDirectories(path:String, stopDirectory:String):Void
	{
		path = Path.standardize(path);
		stopDirectory = Path.standardize(stopDirectory);

		while (path != null
			&& path != ""
			&& path != "."
			&& path != stopDirectory
			&& Path.startsWith(path, stopDirectory)
	...
	private function cleanEmbeddedAndroidAssetOutput(asset:Asset, assetRoot:String):Bool
	{
		if (asset == null || asset.embed != true || asset.type == AssetType.TEMPLATE || asset.resourceName == null || asset.resourceName == "")
		{
			return false;
		}

		var root = Path.standardize(assetRoot);
		var outputPath = Path.standardize(Path.combine(root, asset.resourceName));

		if (!Path.startsWith(outputPath, root))
		{
			Log.error("Refusing to clean embedded Android asset outside output root: " + outputPath);
		}
		...
		Log.info("", " - \x1b[1mRemoving embedded Android asset output:\x1b[0m " + outputPath);

		try
		{
			if (FileSystem.isDirectory(outputPath))
			{
				System.removeDirectory(outputPath);
			}
			else
			{
		...
		{
			Log.error("Embedded asset is still present in Android APK assets output: " + outputPath);
		}
		...
		cleanEmptyDirectories(Path.directory(outputPath), root);
		return true;
	}

	private function cleanEmbeddedAndroidAssetOutputs(sourceSet:String, destination:String):Void
	{
		var checked = 0;
		var removed = 0;
		...
		{
			if (asset == null || asset.embed != true || asset.type == AssetType.TEMPLATE)
			{
				continue;
			}
			...
			var assetRoot = asset.deliveryPackName != null && asset.deliveryPackName != ""
				? destination + "/" + asset.deliveryPackName + "/src/main/assets/"
				: sourceSet + "/assets/";

			if (cleanEmbeddedAndroidAssetOutput(asset, assetRoot))
			{
				removed++;
			}
		}

		Log.info("", " - \x1b[1mChecked embedded Android asset outputs:\x1b[0m " + checked + " embedded, " + removed + " removed");
	}

	private function cleanEmbeddedAndroidModFormatOutputs(sourceSet:String):Void
	{
		var root = Path.standardize(sourceSet + "/assets/assets/mods");

		if (!FileSystem.exists(root) || !FileSystem.isDirectory(root))
		{
			Log.info("", " - \x1b[1mChecked embedded Android mod format outputs:\x1b[0m 0 removed");
			return;
		}
		...
		function recurse(directory:String):Void
		{
			for (entry in FileSystem.readDirectory(directory))
			{
				...
				{
					Log.error("Refusing to clean embedded Android mod output outside root: " + path);
				}
				...
				{
					var format = Path.extension(path).toLowerCase();

					if (embeddedModOutputFormats.exists(format))
					{
						Log.info("", " - \x1b[1mRemoving embedded Android mod format output:\x1b[0m " + path);
						...
						{
							Log.error("Embedded mod format output is still present in Android APK assets output: " + path);
						}

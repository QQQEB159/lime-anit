...
class ProjectXMLParser extends HXProject
{
	...
	private function applyAssetEmbedAndEncryptionSettings(asset:Asset, embedExplicit:Bool, encrypt:Null<Bool>, encryptExplicit:Bool,
			embedFormats:Array<String>, encryptFormats:Array<String>, embedEncryptFormats:Array<String>):Void
	{
		if (asset == null)
		{
			return;
		...
		if (asset.type == AssetType.MANIFEST || asset.type == AssetType.TEMPLATE)
		{
			asset.encrypt = false;
			return;
		}

		var matchesEmbed = assetFormatMatches(asset, embedFormats);
		var matchesEncrypt = assetFormatMatches(asset, encryptFormats);
		var matchesEmbedEncrypt = assetFormatMatches(asset, embedEncryptFormats);

		if (matchesEmbed || matchesEmbedEncrypt)
		{
			asset.embed = true;
			Log.info("", " - \x1b[1mEmbedding asset by format:\x1b[0m " + asset.targetPath);
		}
		...
		else if (matchesEncrypt || matchesEmbedEncrypt)
		{
			asset.encrypt = true;
		}
	}

	private function assetFormatMatches(asset:Asset, formats:Array<String>):Bool
	{
		if (asset == null || formats == null || formats.length == 0)
		{
			return false;
		}
		...
		{
			format = Path.extension(asset.sourcePath);
		}
	...
	private function getOptionalAttribute(element:Access, names:Array<String>):String
	{
		for (name in names)
		{
			var value = element.x.get(name);
	...
	private function parseFormatList(value:String):Array<String>
	{
		...
		for (separator in [",", ";", " "])
		{
			value = StringTools.replace(value, separator, "|");
		}
		...
		for (format in value.split("|"))
		{
			...
			else if (StringTools.startsWith(format, "."))
			{
	...
	private function parseAssetsElement(element:Access, basePath:String = "", isTemplate:Bool = false):Void
	{
		...
		var encrypt:Null<Bool> = null;
		var encryptExplicit = false;
		var embedFormats:Array<String> = [];
		var encryptFormats:Array<String> = [];
		var embedEncryptFormats:Array<String> = [];
		var library = null;
		var targetPath = "";
		var glyphs = null;
		...
		var encryptAttribute = getOptionalAttribute(element, ["encrypt"]);

		if (encryptAttribute != null)
		{
			encrypt = parseBool(encryptAttribute);
			encryptExplicit = true;
		}

		embedFormats = parseFormatList(getOptionalAttribute(element, ["embed-formats", "embedFormats"]));
		encryptFormats = parseFormatList(getOptionalAttribute(element, ["encrypt-formats", "encryptFormats"]));
		embedEncryptFormats = parseFormatList(getOptionalAttribute(element, ["embed-encrypt-formats", "embedEncryptFormats"]));
		...
		else if (!element.elements.hasNext())
		{
			...
			if (!FileSystem.isDirectory(path))
			{
				...
				applyAssetEmbedAndEncryptionSettings(asset, embedExplicit, encrypt, encryptExplicit, embedFormats, encryptFormats, embedEncryptFormats);
			...
			{
				parseAssetsElementLibrary(path, targetPath, "*", "", type, embed, library, glyphs, true, deliveryPackName, embedFormats, encrypt, encryptFormats,
					embedEncryptFormats, embedExplicit, encryptExplicit);
			}
			else
			{
				...
				parseAssetsElementDirectory(path, targetPath, include, exclude, type, embed, library, glyphs, true, deliveryPackName, embedFormats, encrypt, encryptFormats,
					embedEncryptFormats, embedExplicit, encryptExplicit);
			}
		}
		else
		{
			...
			for (childElement in element.elements)
			{
				...
				if (isValid)
				{
					...
					var childEmbedExplicit = embedExplicit;
					var childEncrypt:Null<Bool> = encrypt;
					var childEncryptExplicit = encryptExplicit;
					var childEmbedFormats:Array<String> = embedFormats;
					var childEncryptFormats:Array<String> = encryptFormats;
					var childEmbedEncryptFormats:Array<String> = embedEncryptFormats;
					...
					if (childElement.has.embed)
					{
						...
						childEmbedExplicit = true;
					}

					var childEncryptAttribute = getOptionalAttribute(childElement, ["encrypt"]);

					if (childEncryptAttribute != null)
					{
						childEncrypt = parseBool(childEncryptAttribute);
						childEncryptExplicit = true;
					}

					var childEmbedFormatsValue = getOptionalAttribute(childElement, ["embed-formats", "embedFormats"]);

					if (childEmbedFormatsValue != null)
					{
						childEmbedFormats = parseFormatList(childEmbedFormatsValue);
					}

					var childEncryptFormatsValue = getOptionalAttribute(childElement, ["encrypt-formats", "encryptFormats"]);

					if (childEncryptFormatsValue != null)
					{
						childEncryptFormats = parseFormatList(childEncryptFormatsValue);
					}

					var childEmbedEncryptFormatsValue = getOptionalAttribute(childElement, ["embed-encrypt-formats", "embedEncryptFormats"]);

					if (childEmbedEncryptFormatsValue != null)
					{
						childEmbedEncryptFormats = parseFormatList(childEmbedEncryptFormatsValue);
					}
					...
					applyAssetEmbedAndEncryptionSettings(asset, childEmbedExplicit, childEncrypt, childEncryptExplicit, childEmbedFormats, childEncryptFormats, childEmbedEncryptFormats);
	...
	private function parseAssetsElementDirectory(path:String, targetPath:String, include:String, exclude:String, type:AssetType, embed:Null<Bool>,
			library:String, glyphs:String, recursive:Bool, deliveryPackName:String, embedFormats:Array<String>, encrypt:Null<Bool>, encryptFormats:Array<String>,
			embedEncryptFormats:Array<String>, embedExplicit:Bool, encryptExplicit:Bool):Void
	{
			...
			if (FileSystem.exists(path + "/" + file) && FileSystem.isDirectory(path + "/" + file))
			{
				if (Path.extension(file) == "bundle")
				{
					parseAssetsElementLibrary(path + "/" + file, targetPath + file, include, exclude, type, embed, library, glyphs, true, deliveryPackName,
						embedFormats, encrypt, encryptFormats, embedEncryptFormats, embedExplicit, encryptExplicit);
				}
				else if (recursive)
				{
					if (filter(file, ["*"], exclude.split("|")))
					{
						parseAssetsElementDirectory(path + "/" + file, targetPath + file, include, exclude, type, embed, library, glyphs, true,
							deliveryPackName, embedFormats, encrypt, encryptFormats, embedEncryptFormats, embedExplicit, encryptExplicit);
					}
				}
			}
			else

...
class AssetLibrary
{
	...
	@:noCompletion private var encrypted = new Map<String, Bool>();
	...
	public function getAudioBuffer(id:String):AudioBuffer
	{
		...
		else if (__isEncrypted(id))
		{
			var audioBuffer = AudioBuffer.fromBytes(getBytes(id));
	...
	public function getBytes(id:String):Bytes
	{
		...
		else if (__isEncrypted(id))
		{
			...
			{
				bytes = __createBytesFromClassType(id);
			}
			else if (pathGroups.exists(id))
			{
				bytes = Bytes.fromFile(pathGroups.get(id)[0]);
			}
			else
			{
				bytes = Bytes.fromFile(getPath(id));
			}

			bytes = __processBytes(id, bytes);
	...
	public function getFont(id:String):Font
	{
		...
		{
			var font = Font.fromBytes(getBytes(id));
	...
	public function getImage(id:String):Image
	{
		...
		{
			var image = Image.fromBytes(getBytes(id));
	...
	public function loadBytes(id:String):Future<Bytes>
	{
		...
		else if (__isEncrypted(id))
		{
			if (classTypes.exists(id))
			{
				var bytes = __processBytes(id, __createBytesFromClassType(id));
			...
			var path = pathGroups.exists(id) ? pathGroups.get(id)[0] : getPath(id);
	...
	public function loadFont(id:String):Future<Font>
	{
		...
		else if (__isEncrypted(id))
		{
			return loadBytes(id).then(function(bytes)
			{
				return Font.loadFromBytes(bytes).then(function(font)
				{
					if (font != null)
					{
						cachedFonts.set(id, font);
	...
	public function loadText(id:String):Future<String>
	{
		...
		else if (__isEncrypted(id) || cachedBytes.exists(id) || classTypes.exists(id))
		{
			var bytes = getBytes(id);
	...
	@:noCompletion function __createBytesFromClassType(id:String):Bytes
	{
		#if flash
		return Bytes.ofData(cast(Type.createInstance(classTypes.get(id), []), flash.utils.ByteArray));
		#else
	...
	@:noCompletion function __isEncrypted(id:String):Bool
	{
		return encrypted.exists(id) && encrypted.get(id);
	}

	@:noCompletion function __processBytes(id:String, bytes:Bytes):Bytes
	{
		return __isEncrypted(id) ? AssetEncryption.decryptBytes(id, bytes) : bytes;
	}
	...
	@:noCompletion private function __fromManifest(manifest:AssetManifest):Void
	{
		...
		for (asset in manifest.assets)
		{
			...
			if (Reflect.hasField(asset, "encrypted"))
			{
				encrypted.set(id, Reflect.field(asset, "encrypted"));
			}
